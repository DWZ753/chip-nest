...
- 需要"选哪一格"的地方一律用**选格**按钮：点了之后对话框让开（`stores/picker.ts` 的 `active`），
  回主页面点一个空格即选中，Esc 取消 —— 不要再放「区/层/格」下拉，用户不知道格子编号。
- 只认空格：点到有料的格子给一句结论（「该格已被占用」），不选中。
- 尺寸类设置（每区层/行/列）仍是数字输入，它们不是"选位置"。
