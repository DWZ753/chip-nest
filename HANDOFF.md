...
- 多选：有供应商料号的格子高亮并把料号临时显示在卡片中部；新增「全选 / 清空选择」与每区「选本区」；
  「删除所选」「清除供应商料号」在 0 选中时禁用。
- 产物 v0.3.16：冒烟 exit=0 probe=[200,200,200,'0.3.16'] ui-marker=true。
...
- layout_configs.zone_sizes（JSON [[行,列]...]，缺项沿用 row_count/col_count）：alembic 0005 +
  ...
  新增 test_per_zone_sizes_validation（24 项全绿）。
- 前端：store.zoneGrid()；GridView 按区渲染行列与格数；EditDialog 格选项随所选区收敛；
  仓库布局逐区“名称 + 行 + 列”，全局行/列作为新建区的默认尺寸。
- 编辑元件：区/层/格下拉只显示数字（表头已说明），列宽 2.4:1 重分配，阈值不再挤占。
- BOM：“规划路线”改名“取料顺序”；面板结构收紧；缺料清单改表格式四列（状态/物料/还差/现有）；
  正文小字去 mono 混排。
- 字体：等宽栈补中文回落（PingFang/微软雅黑），消除小字细瘦异体。
- 产物 v0.3.14：冒烟 exit=0 probe=[200,200,200,'0.3.14'] ui-marker=true。
### V0.3.13 八项交互修正 ✅
- 卡片顺序：标称值/封装行在展示标签之前（数值优先）。
- 字体统一：field-label 弃用等宽改正文（中文观感）；chip 默认正文，仅 .mono 显式保留等宽。
- 库存操作合并为一块：当前库存 + 状态色（充足/偏低/告急/缺货）+ 出库｜步进器｜入库 同行。
- 进度条：新增常显轨道；0 库存改为满格红色脉冲告警（不再“条消失=不紧急”）；编辑框同步状态色。
- 区名就地修改：网格区标题悬停出现铅笔，回车保存（PUT /layout zone_names），无需进仓库布局。
- BOM：按钮与标题去掉“（购买清单）”；（开始引导 + 出库流水提示）整行改为已规划后才出现。
- NiceSelect 面板 Teleport 到 body + 视口定位/向上翻转：修复被 overflow 容器裁剪成空小框。
- 产物 v0.3.13：冒烟 exit=0 probe=[200,200,200,'0.3.13'] ui-marker=true。
### V0.3.12 辉光平滑淡出 + README 面向外部 ✅
- 搜索命中动画重做：独立 flash-layer 辉光层做连续线性淡出（opacity 单变量长尾），
  卡片仅轻微上浮（多关键帧缓动），不再出现“一节一节变暗”的阶梯感；时长 2s。
- README.md 重写为面向使用者的外部文档：功能/安装/ESP32/常见问题/许可，删除内部验收与迭代备注。
- 产物 v0.3.12：冒烟 exit=0 probe=[200,200,200,'0.3.12'] ui-marker=true。
...
- 修复：编辑已有元件保存时 patch 漏带 display_tags（新建正常、编辑不生效的真因）；
  现在“展示标签”勾选保存即生效。
- 卡片布局：名称独占一行不再被标签挤压；展示标签移到卡片中部空白区独立成行（任何宽度可见）。
- 多选勾号回到右上角（多选时灯号与数量已隐藏，不再冲突）。
- BOM 取料：“规划路线”面板补不透明背景；“开始取料”后右上角漂浮叉移除，
  “中止”按钮与“已取走，下一步”并列在卡片底部。
- 产物 v0.3.10：冒烟 exit=0 probe=[200,200,200,'0.3.10'] ui-marker=true。
