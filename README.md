...
- **标签双通道**：内部标签负责搜索/筛选；可再从里面挑 1–3 个作为“外部标签”，以高亮胶囊显示在格子上与名称同区，芯片这类只有名字的元件也能一眼认出来。
- **入口分工**：顶栏“仓库布局”管网格尺寸/区名/最近操作与“手工入库”（列对齐表单，先填物料再批量入格）；货架右上“多选”可批量勾选删除；“设置”只管外观与硬件状态。
...
## 打包（2026-09-08 已实跑出包；当前版本 v0.3.8）
...
- `electron/release/ChipNest-Setup-0.3.8.exe` —— **NSIS 安装包**（约 137MB，
