...
- **元件自定义标签**：给任意元件打标签（如“主控”“常用”“板级器件”），卡片上以 #标签 显示（最多 2 个 +N），标签可搜索、可在编辑框维护；无标称值的芯片类元件也能一眼认出来。
- **入口分工**：顶栏“仓库布局”管网格尺寸/区名/最近操作，“设置”只管外观（深浅色/字号）与硬件状态，互不混杂。
...
## 打包（2026-09-08 已实跑出包；当前版本 v0.3.4）
...
- `electron/release/ChipNest-Setup-0.3.4.exe` —— **NSIS 安装包**（约 137MB，
